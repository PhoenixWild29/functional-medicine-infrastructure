...
**Last updated:** 2026-08-15 (later) — **DEMO-DATA EXPANSION SEED + DEMO DOC v2.9.** A schema-verified demo-data expansion seed was generated, committed at `scripts/demo-expansion-seed.sql`, and run against prod by the user. It adds: 3 more Sunrise providers (Dr. Marcus Patel, Dr. Elena Rodriguez, Jamie Fletcher NP — roster realism + F-3 toggle + F-5 primary-provider; **Dr. Chen remains the only auth login/signer**), a second clinic **Blue Cedar Integrative Health** (Dr. Naomi Osei + patient Ruby Sandoval, NM) for the ops multi-tenant view, 8 multi-state Sunrise patients (CA/NY/FL/WA/CO/AZ/IL/GA; primary providers split Chen/Patel/Rodriguez), 17 pharmacy state licenses (every patient state covered; **CA has exactly 2 pharmacies** — Strive + Quick Rx — for the state-license filter beat; TX has all 5), 12 lifecycle orders `DEMO-1001..1012` on V3-catalog formulations spread over ~3 weeks (4 DELIVERED, 2 SHIPPED w/ tracking, 2 PHARMACY_PROCESSING, 2 PAID_PROCESSING, 1 SUBMISSION_FAILED as the ops triage beat, 1 AWAITING_PAYMENT at Blue Cedar), 6 provider favorites (2 each Chen/Patel/Rodriguez), and 1 protocol (**"Menopause Foundation — BHRT"** — Biest + Progesterone + DHEA, 12 weeks, by Rodriguez). Seed is idempotent (ON CONFLICT DO NOTHING, deterministic UUIDs; formulation FKs derived inline via the importer's `md5('formulation:'||lower(name))::uuid` scheme). Demo doc bumped to **v2.9**: new "Demo Cast & Story Beats" subsection + 3 narration beats (patient-state pharmacy filtering via Jordan Rivera CA; the F-3 My-patients/All-clinic toggle now meaningful; living pipeline incl. the DEMO-10xx failed-order triage beat) + updated seed-data table + Part 3B provider-selection fix. Docs landed via the `docs/demo-expansion-seed-v29` PR (not merged at write time). **Open items unchanged:** Lauren 503B email, local PDF/DOCX regeneration + git pull, secrets rotation out of `.env.local`.

**Prior: 2026-08-15** — **CATALOG-RESEED FALLOUT FIXED + R10 WALKTHROUGH PASS.** Production at commit `9771f1d`+. The user found that the V3 catalog reseed had orphaned the provider quick actions — favorites 404'd on click (foreign keys still pointed at old-catalog rows) and protocol templates loaded with $0.00 stub pricing. Remediation: a **prod data fix** (favorites/protocol items remapped to the reseeded V3 catalog; new product **Ketotifen Capsule 1mg** added at $22 wholesale) plus three PRs merged — **#109** (protocols load real wholesale pricing + clinic-default markup; stale-favorite hardening so dangling FKs degrade gracefully instead of 404ing), **#110** (Review & Send signature/validation feedback: disabled-send hint "Sign in the signature box above to enable sending" replacing the silent no-op), **#111** (bundle-link recovery: "Part of a Payment Bundle" drawer panel + Copy Bundle Payment Link via new `/api/orders/[id]/group-link`, plus a specific "part of a combined payment bundle" message when an old solo link is opened for a bundled order; a tsc pass during the PR caught a polling regression before merge). The **R10 walkthrough ran against prod** (2026-08-15, all roles) — report at `docs/qa-reports/qa-poc-demo-round10-report.md`: **PASS across provider / clinic-admin / ops**, 0 open blockers, including a live 4-prescription bundle checkout render at $444.80 (payment intentionally not executed — live Stripe key). Demo doc bumped to v2.8 to match. **Open items unchanged:** Lauren 503B email, local PDF/DOCX regeneration + git pull, secrets rotation out of `.env.local`.
