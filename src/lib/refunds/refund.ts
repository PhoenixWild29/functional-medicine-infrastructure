...
//
// Parameters mirror the existing single-order refund exactly —
// { payment_intent } — and a bundle member adds only the `amount` a
// partial refund requires. No other refund parameter is set. The
// idempotency key is a request option, not a refund parameter: every
// attempt for an order uses the same key, so a retry of a refund Stripe
// already made returns that refund instead of making a second one.
//
...
type PendingResult =
  | { ok: true; pendingSince: string | null; target: RefundTarget | null }
  | { ok: false; error: string }
...
export async function pendingRefund(supabase: Supabase, order: RefundableOrder): Promise<PendingResult> {
  ...
  return { ok: true, pendingSince: row?.created_at ?? null, target }
}
...
export async function issueRefund(stripe: Stripe, orderId: string, target: RefundTarget): Promise<IssueResult> {
  const params: Stripe.RefundCreateParams = target.amountCents == null
    ? { payment_intent: target.paymentIntentId }
    : { payment_intent: target.paymentIntentId, amount: target.amountCents }
  try {
