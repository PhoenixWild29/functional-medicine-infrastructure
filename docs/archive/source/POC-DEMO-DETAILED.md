...
**Version:** 2.26 | **Date:** September 25, 2026
**Application:** https://functional-medicine-infrastructure.vercel.app
**Duration:** 30–45 minutes (with discussion)

> **What's new in v2.26 (2026-09-25):** **Cycling doses count the days the patient actually doses.** A cycling line ("5 days on / 2 days off") was sized as if the patient dosed every day: 10 units for 30 days came out 3.0 mL and the 5 mL vial. It is now **22 dosing days, 2.2 mL, the 2.5 mL vial**. The rule, shown on screen: full cycles × on-days plus the on-days in the last partial cycle, **with the course starting on an on-day**, so the patient is never short. Days supply is still the calendar span. The dose step shows **"22 dosing days in 30 days (5 days on / 2 days off, starting on an on-day)"** under the cycle, the price step shows it under Days supply / Dispense, and each Review card shows the count beside its days supply. In Cycling mode the cycle length *is* the duration (the separate Duration dropdown is gone there); it defaults to the 6 weeks already on screen, and **(ongoing)** sizes from the package like Standard's Ongoing. **Mon-Fri** follows the same rule: 30 days is now 22 doses (was 21); 28 days is still 20. The pattern is stored with the order, so a reopened draft, a refill, the **BPC-157 daily cycling** favorite (now opening in Cycling mode, from its chip and from the dose step's Common doses) and a protocol's cycling item all come back as the same cycling line, and the pharmacy payloads, fax and Rx PDF carry the pattern beside the dispense. A cycling order written before the pattern was stored is never refilled as daily dosing: the refill stops on the dose step in Cycling mode and asks for the days on and off, showing the old directions. New optional beat after step 29.
...
> **What's new in v2.22 (2026-09-18):** **WO-101b — a pharmacy lists only the vial sizes it prices.** Found on prod: Quick Rx's option card read "3 × 1 mL vials · Available: 1 mL vial, 3 mL vial" for Semaglutide 20 units weekly for 90 days, but no 3 mL vial had a price. The size list and the suggestion now come from the same place — the pharmacy's priced packages — so a pharmacy with one priced vial shows "Available: 1 mL vial" and nothing else, and no size without a known price is shown, selectable or suggested. Strive (three priced vials) is unchanged. Step 30 updated.

> **What's new in v2.21 (2026-09-17):** **WO-104 — Favorites are a drug with its common doses; they open the dose step, not a free-text box** (Gina Rooks, 2026-09-11: *"it would be nice to still be able to revert to the Rx builder drop downs rather than editing a free text box … I'd prefer to see semaglutide injection, click it and see the most common prescription doses written out"*). Still three steps; nothing new to type. (a) **One card per drug + formulation + pharmacy (step 13).** Migration `20260917000001` merges favorites that were one row per dose: Dr. Chen's Semaglutide is now a single **Semaglutide** card with chips **10 units (0.5 mg) weekly · 20 units (1.0 mg) weekly · 40 units (2.0 mg) weekly · Custom** (the migration also adds the clinic's 20 and 40 unit doses to the demo seed). The button still reads **Favorites (10)** — ten cards. (b) **A chip opens the dose step** with every dropdown filled — amount, unit, frequency, timing, duration — and the sig generated from them; **Custom** opens the dose step with the formulation and pharmacy chosen and the dose empty. Then Continue prices as before. The same chips also appear on the dose step whenever that formulation is selected (step 26). (c) **Sorted by category** in a fixed order (Peptides, Hormones, Weight Management, Sexual Health, Longevity, …, Other), A–Z inside each group; the **Mine** checkbox still narrows it. (d) **Recent** strip at the top of the panel: the last 8 formulations the selected provider prescribed, each with **Make favorite**. (e) **Patient favorites** — ☆ Save as favorite (step 31, 33, 40) now offers **For the practice** (default) or **Only for Alex Demo**; a patient's own favorites show first, under **For Alex Demo**, and only when that patient is selected. The ☆ name field defaults to the drug (**"Semaglutide Injectable 5 mg/mL"**); saving a dose for a drug the clinic already has adds a chip to that card instead of a second card. Steps 13, 26, 31 and the presenter line after step 13 updated.

> **What's new in v2.20 (2026-09-16):** **WO-102 — Shipping, once per pharmacy, everywhere a price appears** (Gina Rooks 2026-09-11: *"I don't see [shipping] listed anywhere with the med pricing or when you get to review and sign … you pay shipping more than once"*). Still three steps; nothing new to type. (a) **Rates live on the pharmacy:** Strive **$9 standard / $22 cold chain**, Quick Rx **$12 / $25**. The fee is charged **once per pharmacy per order**, never per prescription, and if any item going to a pharmacy is cold chain the cold-chain fee covers all of them. (b) **Margin page (step 33, 39)** — a **Shipping** line under the pharmacy: Semaglutide at Strive reads **"Shipping (cold chain): $22.00"**; the Testosterone Cypionate added to the same Strive order reads **$0.00 — it ships with the other Strive prescription**. Shipping is **outside the margin**: the platform fee and clinic margin are unchanged by it. (c) **Review (step 40)** — totals now read **Subtotal $286.00 · Shipping — Strive Pharmacy (cold chain, 2 items in one shipment) $22.00 · Platform fee $21.45 · Clinic payout $121.55 · Patient total $308.00**. If a session spans **two pharmacies**, an amber notice says what the split costs — e.g. **"2 pharmacies → 2 shipping charges ($34.00). Route all to Strive Pharmacy to save $12.00."** — with a one-click **Route all to Strive Pharmacy**. It is offered only when every item is available at that pharmacy and it is licensed in the patient's state; if re-routing changes a medication price the notice names the change and the net, and it never offers a re-route that costs more overall. (d) **Sign page (step 54)** and **bundle (step 60)** include shipping: the bundle reads **"2 prescriptions · $308.00"**. (e) **Checkout (Part 4A)** shows **Prescriptions $286.00 · Shipping $22.00 · Amount due $308.00**; the Stripe charge is subtotal + shipping and the platform fee is never taken on shipping. (f) **Settings (step 67)** — a clinic can tick **Absorb shipping costs** to pay shipping out of its payout instead of passing it to the patient (off by default). Migration `20260916000001`. Steps 33, 39, 40, 54, 60, 67 and Part 4A updated.
...
>
> **(a) SAFETY — no card is ever entered.** Prod is on **live** Stripe keys. The old Pre-Demo item 3 told the presenter to type `4242 4242 4242 4242` into the real checkout page in Part 4B — on live keys that hard-declines on stage, and substituting a real card would put through a **real ~$308.00 charge ($286.00 + $22.00 shipping) with a real Connect payout split**. Every instruction to enter a card number has been removed from this document and from `POC-DEMO-QUICKSTART.md`. Pre-Demo item 3 is now a warning box. **Part 4B is "render and narrate"** with a scripted line that makes stopping look deliberate. **Part 4C** (`/checkout/success`) is unreachable without paying and is now marked **describe-only, not shown live**.
>
...
> - **A real card will actually charge.** Typing a personal or corporate card into the Part 4 checkout puts through a **real ~$308.00 payment** ($286.00 + $22.00 shipping), against a **real Stripe Connect payout split** to the clinic's connected account. That is a real refund, a real reversal, and a real conversation you do not want to have after a sales call.
>
...
13. Click **Favorites (10)** — a panel opens under the search box (the count on the button reads **Favorites (10)**; the number is clinic-wide and counts cards, not doses). **v2.21:** at the top, a **Recent** strip shows up to 8 formulations Sarah Chen prescribed most recently, each with **Make favorite** (its contents depend on what has been prescribed in the demo project, so don't script them). Below it the favorites are **grouped by category, A–Z inside each group**:

    - **Peptides** — BPC-157 daily cycling (cycling badge)
    - **Hormones** — Biest 80/20 — Menopause Std · Estradiol 0.1% Cream · Standard TRT — Cyp 200mg · TRT Cyp 200 — Weekly
    - **Weight Management** — **Semaglutide**
    - **Sexual Health** — Tadalafil 10 Troche
    - **Longevity** — NAD+ Longevity
    - LDN Starter — Titration (titration badge) and LDN 4.5 Maintenance sit in the naltrexone ingredient's category (**Autoimmune** or **Other**, whichever the catalog carries) — read what the screen shows.

    **Read the Semaglutide card out loud.** One card — formulation and **Strive Pharmacy** under the name — and the clinic's common doses written out as chips: **10 units (0.5 mg) weekly · 20 units (1.0 mg) weekly · 40 units (2.0 mg) weekly · Custom**. The mg is computed from the syringe units and the 5 mg/mL concentration, never typed. *"You don't save every unit dose of semaglutide as its own favorite. You save semaglutide, and the practice's standard doses live under it."* A favorite whose dose unit the builder has no dropdown for (the estradiol cream in grams, the tadalafil troche) shows only **Custom**.

    **Optional 20-second proof:** click **20 units (1.0 mg) weekly**. The panel closes and you are on the **dose step** — not a free-text box — with **20 · units · Once weekly · In the morning** already selected and the sig generated underneath. *"If I want 25 units I change the number; the dropdowns are all still there."* Then retype **"Sema"** in the search to start step 24 clean. Point at the **Mine** checkbox (narrows the list to the selected provider's own favorites) and the **+ New** button (closes the panel and puts the cursor in the search box). Hover a card: the **pencil** edits the name, pharmacy, who it is for and the list of doses (add, remove, change — each dose previews its mg); the **trash** icon still asks for a confirm before removing the favorite for the whole clinic.

> "Provider favorites let you reorder common prescriptions in one click. Pick the drug, pick the dose — and you land on the dose step with every dropdown already set, so the provider can still adjust anything before going to pricing. Nothing is retyped and nothing is a free-text sig."

> **Presenter line — say this before they ask:** "You'll notice favorites here that Dr. Chen didn't save. Favorites are **clinic-wide**, not per-provider — Dr. Patel's TRT setups and Dr. Rodriguez's BHRT setups show up here too, grouped by category. That's intentional. When a new provider joins a practice, they inherit the practice's prescribing patterns on day one instead of rebuilding them from scratch." (A prospect **will** ask why another doctor's favorite is on this screen. Answer it first.)
...
>
> **Part 3C-1 may have left you on Jordan Rivera (CA).** Everything from step 24 onward assumes **Alex Demo (TX)**. If you are still on Jordan, the Strive/Semaglutide path in step 30 may behave differently and the $190/$96/$286.00 pricing anchors (and the $22.00 Strive shipping, $308.00 total) will not match.
>
...
26. Select the **"Semaglutide Injectable 5 mg/mL"** card → **Point out the Structured Sig Builder**:
    ...
    - **v2.21:** a **Common doses** row of chips above the dose fields — the clinic's saved Semaglutide doses (10 / 20 / 40 units). A chip fills every dropdown; typing your own dose still works. For the script, set the fields by hand as in step 27.
...
29. **Show Titration mode** (**v2.23 / WO-105**) — click the "Titration" toggle. The amber panel is now a **step table**: one row per dose change, each with dose, unit, frequency and weeks, plus **+ Add step** and **Remove**. Enter 10 units weekly for 4 weeks, add a step at 20 units for 4 weeks, add a third at 40 units for 4 weeks. Each row shows its own quantity — **0.4 mL / 0.8 mL / 1.6 mL** — and the panel totals **"Total 2.8 mL over 84 days"**. The single Dose and Duration fields disappear in this mode: the steps are the dose and the duration. The sig reads **"Weeks 1–4: inject 10 units subcutaneous once weekly. Weeks 5–8: … Total dispense 2.8 mL over 84 days."**

> "This is what a titration actually is — do this for the first four weeks, then this for the next four. The app adds it up: 2.8 mL, not the 4.8 mL you'd dispense if you wrote the script at the top dose for twelve weeks, which is what providers do today when the software can't take a schedule. The pharmacy gets the steps as fields, not a free-text 'titrate up as tolerated' line they push back on, and the patient sees the same schedule in plain language at checkout with the total restated at the bottom."

**Optional 30-second beat — Cycling (v2.26).** Click **Cycling**. With 10 units once daily, the blue panel reads **5 days on / 2 days off, Cycle for 6 weeks** and, underneath, **"30 dosing days in 42 days (5 days on / 2 days off, starting on an on-day)"**. Change the cycle to **30 days**: it reads **22 dosing days in 30 days**, and Strive's row suggests the **2.5 mL vial**, not the 5 mL. *"The patient takes it on 22 of those 30 days, so we dispense 22 doses. We assume the course starts on an on-day, which is the most doses that span can hold, so the patient never runs short."* Click back to **Standard** before step 30.

Two things worth showing if asked: applying the **LDN Starter — Titration** favorite re-opens the step table with its steps (before v2.23 it silently flattened to a standard sig), and entering steps that cross capsule strengths — ketotifen 0.1 mg then 0.5 mg — is **refused** with a message to add a second line, because those are two compounded products at two prices. It never quietly computes one wrong quantity.

30. Click back to **Standard** mode. Select **Strive Pharmacy** in the Pharmacy & Pricing section. **v2.18:** Strive's row shows **$95.00** with **1 mL vial** under the price, and below it every vial Strive sells with its own price — **"1 mL vial $95.00 · 2.5 mL vial $165.00 · 5 mL vial $285.00"**. *"The app worked out that 10 units a week for 30 days is 0.4 mL, so the 1 mL vial is enough — and that's the price you're looking at."* (**v2.22:** a pharmacy with a single priced vial shows **"Available: 1 mL vial"** — only the size it actually prices. Sizes a pharmacy lists without a price are never shown or suggested; for a larger dose it suggests several of its priced vial, e.g. Quick Rx at 20 units weekly for 90 days reads **"3 × 1 mL vials · $285.00"**.)
31. **v2.18: there is no Quantity dropdown for Strive** — the suggested vial from step 30 *is* the quantity, and it can be changed with its price on the next screen. Leave refills at 0. (For a pharmacy with a single package, Quantity is still pre-filled with the smallest vial that covers 30 days, as in v2.17.) **Point out the ☆ button** next to "Continue — Set Retail Price" — **v2.21:** clicking it opens a name field already filled in with the drug, **"Semaglutide Injectable 5 mg/mL"**, and a choice of **For the practice** (selected) or **Only for Alex Demo**; saving adds this dose as a chip under the clinic's existing card for that drug and pharmacy, or creates the card. The same ☆ appears again on the price page and on every Review card, so a favorite can be saved at whichever point the provider decides it is worth keeping. Don't save one during the scripted run — the seeded list is what the Favorites beat counts on.
...
39. Cascade: salt form **Cypionate** (auto-skips as the only option) → dosage form **Injectable Solution** → route **Intramuscular** → formulation **"Testosterone Cypionate Injectable 200 mg/mL"** → set dose + frequency. Select Strive Pharmacy, set retail price. The Testosterone Cypionate 200 mg/mL wholesale is **$48**, and the same **2x multiplier** the demo uses puts its retail at **$96.00**. **v2.20:** its margin page reads **"Shipping (standard): $0.00 — ships with the other Strive Pharmacy prescription in this session ($22.00 once)"**. Click **"Review & Send (2)"**
...
40. **Point out the batch review page:**
    ...
    - **Totals (v2.20):** **Subtotal (2 prescriptions) $286.00**, **Shipping — Strive Pharmacy (cold chain, 2 items in one shipment) $22.00** (one line per pharmacy; cold chain covers the Testosterone vial too), **Platform fee (15% of margin, not charged on shipping) $21.45**, **Clinic payout $121.55**, **Patient total $308.00**. *"Two prescriptions, one box, one shipping charge."*
    - **v2.20 — the multi-pharmacy notice does not appear here**, because both items go to Strive. If you want to show it, see the optional box after step 40.
...
> **Optional beat — two pharmacies, two shipping charges (v2.20).** Click **Edit** on Testosterone Cypionate, pick **Quick Rx Pharmacy** instead of Strive, continue and save back to Review. The totals now show **two** shipping lines (Strive cold chain $22.00 + Quick Rx standard $12.00) and an amber notice reads **"2 pharmacies → 2 shipping charges ($34.00). Route all to Strive Pharmacy to save $12.00."** Click **Route all to Strive Pharmacy**: the Testosterone line moves back, shipping returns to **$22.00 once**, and the notice disappears. *"You might get a better price somewhere else — the app shows whether it survives a second shipping charge, and fixes it in one click."* If moving an item would change its price, the notice says so with the net before you click, and it never suggests a move that costs more overall.
...
> **Know before you log in: there are two drafts and you will sign them together.** An N-prescription session saves as N separate draft orders (see step 43), so the scripted flow leaves **2** drafts in the queue — Semaglutide and Testosterone Cypionate. **v2.24 (WO-99):** Dr. Chen signs both **at once** — one page, one signature, one click, one authenticator code for the controlled line. There is no second pass.

50. **Log in as provider:** `dr.chen@sunrise-clinic.com` / `POCProvider2026!`
51. Click the **"Drafts"** tab on the dashboard — it reads **Drafts 2**. Above the table: **Sign all (2)**. Each of Dr. Chen's draft rows has a checkbox (ticking rows offers **Sign selected (k)**).
...
> "The Drafts tab lives on the shared dashboard and is visible to both clinic_admin and provider roles — anyone in the clinic can see what's pending signature, not just providers. The provider just happens to be the one who can act on it. The MA can watch the queue; she just can't clear it. And 'Sign all' counts only what's Dr. Chen's to sign — a draft saved under another provider isn't in that number."

52. Click **Sign all (2)**. The batch sign page opens (`/new-prescription/sign?orders=…`) with **both drafts selected** under **Alex Demo**.

> "Same signing surface that bounced the MA out a minute ago. Dr. Chen walks straight in — and she's looking at the whole visit, not one prescription at a time."

53. **Point out** the page, top to bottom: the **red EPCS banner** naming Testosterone Cypionate (controlled — the code will be asked for once, for the batch); **Alex Demo** with the green **NKDA** chip; the drug interaction check across both prescriptions; **Draft lines (2)**, each with its checkbox, **Edit**, **Remove** and a collapsed **Rx details** row; and the totals — **Subtotal $286.00 · Shipping — Strive Pharmacy (2 items, once) $22.00 · Platform fee · Clinic payout · Patient total — one payment link $308.00** (the $190.00 Semaglutide assumes the **2×** tap; it defaults to $133.00). One signature pad, one **Sign & Send 2 Prescriptions**, disabled with the hint **"Sign in the signature box below to enable sending."**

> "No silent failures. Every check the review page ran runs here too, for every prescription on the page — allergies, interactions, the Rx details, the price. If any of them can't run, or any line can't be sent, the button stays off for the whole batch and the page says which line and why."

54. **Optional — the draft lines are still editable here (v2.17 / WO-98).** Click **Edit** on Semaglutide: the builder reopens with Semaglutide, Strive, 10 units and once weekly already selected — change the dose to 15 units, continue, and **"Save Changes to Draft"** brings you back to this page with the line updated and the **same order id**. **"+ Add prescription"** appends a line (patient and provider pinned). Open the order drawer afterwards and the timeline shows **"Draft edited · changed dose, sig …"** with the provider as the actor. Remove is a soft delete. Skip it if time is short.

55. **Sign** on the pad — a real signature, **three strokes across the pad**. (A single dot is refused: the hint reads *"Sign with at least 3 strokes — a dot or a single line is not a signature."*) Watch **"Signature captured"** appear and the button enable. Click **Sign & Send 2 Prescriptions** — once.

> **EPCS 2FA:** the EPCS two-factor modal appears **once, for the batch**, because Testosterone Cypionate is DEA Schedule 3 — a red **"EPCS Two-Factor Authentication Required"** header, the controlled prescription listed with its Schedule badge, a 6-digit TOTP input citing DEA 21 CFR 1311, and **"Verify & Sign"** / **"Cancel"**. Read the current code from the authenticator app you set up before demo day and enter it. **Cancel signs nothing** — neither prescription — which you can show first if you want the point to land.

> "Controlled substance, so the platform asks the physician for a second factor before it will accept the signature. This is DEA 21 CFR 1311, and it is not optional — the code is checked on the server in the same request that signs, so there's no way to sign a Schedule 3 on this platform without it. One code covers the batch, and the audit record names every controlled prescription it signed."

56. You land on the dashboard. **Both Alex Demo orders read "Awaiting Payment"**, the Drafts tab has emptied and disappeared, and the patient has been sent **one** payment link covering both. Confirm that on the dashboard before moving on.

> **Say what just happened; don't hedge it.** One signature signed both prescriptions, and the record keeps **one signature per order** — each carries the same signature and the same timestamp. All or nothing: if either prescription had failed a check, neither would have been signed. *"Gina asked for this in so many words: sign all of a patient's prescriptions at once. One visit, one signature, one link."*
...
> **Deep links still work.** The drawer's **Review & Sign This Prescription** (and any old `/new-prescription/sign/<order-id>` link) opens this same page with **that** draft selected; the patient's other drafts are listed unselected with a note that signing them separately sends a second link and can charge shipping again, and a one-click **Select all for Alex**.

> **Optional beat — "Sign as me" (v2.17 / WO-100, only if a draft belongs to another provider).** The two scripted drafts are Dr. Chen's, so this does not appear in the script. If the MA saved a draft under **Dr. Patel** (say, during an improvised 3B), it is **not counted** in Dr. Chen's **Sign all** and has no checkbox. Open it from **All clinic orders → Drafts → Review & Sign This Prescription**: under the patient, an amber **"This draft is assigned to Marcus Patel — You are signed in as Sarah Chen"** panel with a **Sign as me** button, and no signature pad for it. Click it: every line of that draft moves to Dr. Chen, the audit trail records the reassignment from Patel, and the page re-renders with those drafts signable under her name. *"Dr. Patel is out today. Dr. Chen takes the draft over under her own name and license — and the record says exactly who took it from whom. Nothing is ever signed under someone else's name."*

### 3I — Phase C: Combine and Send

> "Those two prescriptions are siblings — same patient (Alex Demo), same provider (Dr. Chen). Before Phase C, that meant two separate payment links the patient had to open and pay one at a time. **v2.24:** signing them together already bundled them — one payment group, one checkout."
...
58. **Point out the "Part of a Payment Bundle" panel** — it is already there: **"2 prescriptions · $308.00"** ($286.00 of prescriptions plus $22.00 Strive shipping, charged once). There is no "Combine" step for these two: they were signed together, so they were bundled at signing.
59. Click **"Copy Bundle Payment Link."**
60. One bundled checkout link covering both prescriptions is now on your clipboard — Semaglutide $190.00 + Testosterone Cypionate $96.00 + $22.00 shipping. This is the link you'll paste in Part 4.

> "One link, both prescriptions, one payment. The patient taps once and pays a single combined total instead of juggling two links — and nobody had to remember to combine them, because signing them together did it. **Combine and Send** is still in the drawer for orders that were signed separately: two Awaiting-Payment orders for the same patient and provider can be bundled after the fact exactly as before."
...
63. **Copy the patient checkout URL.** For the bundled pair you already have it from step 59. For any single order: on the clinic dashboard, click an order showing **"Awaiting Payment"** to open the order drawer, then click the emerald **"Copy Payment Link"** button — the checkout URL is on your clipboard.
...
>
> **No card number is entered in Part 4. None.** A test card will hard-decline; a real card will really charge ~$308.00 (with shipping) with a real Connect payout split. **We render the page, we narrate it, and we stop.** Part 4B gives you the words.
...
2. **Point out:**
   ...
   - **"Prescription Bundle · 2 prescriptions · $308.00"** — the two sibling Rx now share one checkout instead of two separate links
   - **v2.20 — line items under the amount:** **Prescriptions $286.00** and **Shipping $22.00**. Shipping is its own line, charged once for Strive.
   - Two generic line items, each labeled **"Prescription Service"** — NOT the medication name
   - A single **combined total of $308.00** — the two retail prices set in Part 3E (Semaglutide $190.00 + Testosterone Cypionate $96.00; the **$190.00** assumes the **2×** tap — it defaults to $133.00) plus **$22.00** shipping. The patient pays this one combined amount, once.
...
> "Once. A session of N prescriptions saves as N separate draft orders, and the provider signs them together: one page that shows every one of them with its checks, one signature, and one EPCS code if any of them is a controlled substance. The record still keeps one signature per prescription — each order carries the same signature and the same timestamp — so every prescription has its own attestation. It is all or nothing: if any prescription can't be sent, none is signed, and the page says which one and why. And a provider can only ever sign their own drafts; another provider's draft has to be taken over by name first."
>
> *Presenter note (v2.24): this is WO-99, built from Gina Rooks' own request. Show it in Part 3H rather than describing it.*
